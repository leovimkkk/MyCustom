...
import com.xwjr.utilcode.customview.datePicker.WheelPicker;
...
public class AddressPicker extends WheelPicker {
    ...
    private String selectedProvinceCode = "", selectedCityCode = "", selectedCountyCode = "";
    ...
    public void setSelectedItem(String province, String city, String county) {
        for (int i = 0; i < provinceList.size(); i++) {
            ...
            if (pro.contains(province)) {
                ...
                LogUtils.i("init select province: " + pro);
                break;
            }
        }
        ...
        for (int j = 0; j < cities.size(); j++) {
            ...
            if (cit.contains(city)) {
                selectedCityIndex = j;
                LogUtils.i("init select city: " + cit);
                break;
            }
        }
        ...
        for (int k = 0; k < counties.size(); k++) {
            ...
            if (cou.contains(county)) {
                ...
                LogUtils.i(AddressPicker.class.getSimpleName(), "init select county: " + cou);
                break;
            }
        }
        LogUtils.i(AddressPicker.class.getSimpleName(), String.format("init select index: %s-%s-%s", selectedProvinceIndex, selectedCityIndex, selectedCountyIndex));
    }
    ...
    public void onSubmit() {
        ...
        String selectedProvinceCode = "", selectedCityCode = "", selectedCountyCode = "";

        for (int i = 0; i <data.size() ; i++) {
            Province province = data.get(i);
            if (province.getAreaName().equals(selectedProvince)){
                selectedProvinceCode=province.getAreaId();
                for (int j = 0; j < province.getCities().size(); j++) {
                    City city = province.getCities().get(j);
                    if (selectedCity.equals(city.getAreaName())){
                        selectedCityCode=city.getAreaId();
                        for (int k = 0; k <city.getCounties().size(); k++) {
                            County county = city.getCounties().get(k);
                            if (selectedCounty==county.getAreaName()){
                                selectedCountyCode=county.getAreaId();
                            }
                        }
                    }
                }
        ...
        if (onAddressPickListener != null) {
            ...
            onAddressPickListener.onAddressPickedCode(selectedProvinceCode,selectedCityCode,selectedCountyCode);
        }


        System.out.println(selectedProvinceCode+""+selectedCityCode+""+selectedCountyCode);
    }
    ...
    public interface OnAddressPickListener {
        ...
        void onAddressPickedCode(String provinceCode, String cityCode, String countyCode);

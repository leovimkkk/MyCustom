...
public class MyProgressCircle2 extends View {
    ...
    float width;
    float height;
    float borderWidth = 4;//圆弧宽度
    private float startAngle = 270;//开始绘制的角度
    private float totalAngle = 360;//开始绘制的角度
    ...
    private String currentProgressText = "0";

    private int[] colors = new int[]{
            Color.parseColor("#ff6f28"),
            Color.parseColor("#ff6f28"),
            Color.parseColor("#ff6f28"),
            Color.parseColor("#ff6f28")
    };
    ...
    public MyProgressCircle2(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public MyProgressCircle2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void init(Context context) {
        width = (float) (ViewUtil.getWindowWidth(context) * ViewUtil.getWeight(90.0));
        height = (float) (ViewUtil.getWindowWidth(context) * ViewUtil.getWeight(90.0));
        progress = 80;
...
//        drawCircle(canvas);
    ...
    public void drawBitmapBackground(Canvas canvas) {
        ...
        RectF oval1 = new RectF(borderWidth / 2,
                borderWidth / 2,
                (width - borderWidth / 2),
         ...
         */
        //绘制默认灰色的圆弧
        canvas.translate(0, 0);//这时候的画布已经移动到了中心位置
    ...
    private void drawCircle(Canvas canvas) {
        ...
        canvas.translate(width / 2, height);//这时候的画布已经移动到了中心位置
        ...
        paintCircle.setTextSize(sp2px(12));
        Paint.FontMetrics fontMetrics = paintCircle.getFontMetrics();
        float top = fontMetrics.top;//为基线到字体上边框的距离,即上图中的top
        float bottom = fontMetrics.bottom;//为基线到字体下边框的距离,即上图中的bottom
        textY = (textY - top / 2 - bottom / 2);
        canvas.drawText(String.valueOf((int) (currentAngle * 100 / 180.0)) + "%", -textX, -textY, paintCircle);

        canvas.rotate(currentAngle);
        paintCircle.setColor(Color.parseColor("#ff5628"));
        canvas.drawCircle(-(width - borderWidth) / 2, 0, 16, paintCircle);
        paintCircle.setColor(Color.parseColor("#ffffff"));
        canvas.drawCircle(-(width - borderWidth) / 2, 0, 8, paintCircle);
    ...
    private void drawText(Canvas canvas) {
        ...
        paintCircle.setStyle(Paint.Style.FILL);//设置填充样式
        paintCircle.setAntiAlias(true);//抗锯齿功能
        paintCircle.setColor(Color.parseColor("#a4aeb4"));
        canvas.translate(width / 2, height / 2);//这时候的画布已经移动到了中心位置
        paintCircle.setTextAlign(Paint.Align.CENTER);
        paintCircle.setTextSize(sp2px(14));
        Paint.FontMetrics fontMetrics = paintCircle.getFontMetrics();
        float top = fontMetrics.top;//为基线到字体上边框的距离,即上图中的top
        float bottom = fontMetrics.bottom;//为基线到字体下边框的距离,即上图中的bottom
        ...
        if (currentAngle <= 0) {
            canvas.drawText(currentStatus, 0, textY, paintCircle);
        } else {
            canvas.drawText(currentProgressText + "%", 0, textY, paintCircle);
        }
    }
...
//        this.currentCount = currentCounts > maxCount ? maxCount : currentCounts;
        currentProgressText = String.valueOf((int) currentCounts);
        float last_angle = totalAngle * (currentCounts / 100);//最后要到达的角度
        lastAngle = currentAngle;//保存最后绘制的位置
        if (currentCounts > 0) {
            setAnimation(lastAngle, last_angle, 0);
        } else {
            currentAngle = currentCounts;
            invalidate();
        }
    }

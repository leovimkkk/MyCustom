 ...
 * @author zhy
 */
public class GestureLockViewGroupTop extends RelativeLayout {

    private static final String TAG = "GestureLockViewGroupTop";
    /**
     * 保存所有的GestureLockView
     */
    private GestureLockViewTop[] mGestureLockViews;
    /**
...
//    private Integer[] mAnswer = {0, 1, 2, 5, 8};
     ...
     */
    private int mFingerOnColor = 0x000000;
    /**
     * GestureLockView手指抬起的状态下内圆和外圆的颜色
     */
    private int mFingerUpColor = 0x000000;
    ...
    public GestureLockViewGroupTop(Context context, AttributeSet attrs) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mGestureLockViews == null) {
            mGestureLockViews = new GestureLockViewTop[mCount * mCount];
...
//            mPaint.setStrokeWidth(mGestureLockViewWidth * 0.29f);
            ...
            for (int i = 0; i < mGestureLockViews.length; i++) {
                ...
                mGestureLockViews[i] = new GestureLockViewTop(getContext(),
                ...
                lockerParams.setMargins(leftMagin, topMargin, rightMargin,
                        bottomMargin);
                mGestureLockViews[i].setMode(GestureLockViewTop.Mode.STATUS_NO_FINGER);
...
//                GestureLockViewTop child = getChildIdByPos(x, y);
//                if (child != null) {
//                    int cId = child.getId();
...
//
//                    }
//                    child.setMode(GestureLockViewTop.Mode.STATUS_FINGER_ON);
//                }
//                // 指引线的终点
//                mTmpTarget.x = x;
//                mTmpTarget.y = y;
...
//                    mOnGestureLockViewListener.passwordList(mChoose);
...
//    }
    ...
    private void changeItemMode() {
        for (GestureLockViewTop gestureLockView : mGestureLockViews) {
            if (mChoose.contains(gestureLockView.getId())) {
                gestureLockView.setMode(GestureLockViewTop.Mode.STATUS_FINGER_UP);
            }
        }
    }
     ...
     * 转成错误显示
     */
    public void changeErrorMode() {
        mPaint.setColor(getResources().getColor(R.color.circle_border));
        for (GestureLockViewTop gestureLockView : mGestureLockViews) {
            if (mChoose.contains(gestureLockView.getId())) {
                gestureLockView.setMode(GestureLockViewTop.Mode.STATUS_ERROR);
            }
        }
    }
    ...
    public void reset() {
        ...
        mPath.reset();
        for (GestureLockViewTop gestureLockView : mGestureLockViews) {
            gestureLockView.setMode(GestureLockViewTop.Mode.STATUS_NO_FINGER);
...
//    }
    ...
    private boolean checkAnswer() {
        if (answerList.size() != mChoose.size())
            return false;
        for (int i = 0; i < answerList.size(); i++) {
            if (answerList.get(i) != mChoose.get(i))
                return false;
        }
        return true;
    }
     ...
     * @return
     */
    private GestureLockViewTop getChildIdByPos(int x, int y) {
        for (GestureLockViewTop gestureLockView : mGestureLockViews) {
...
//    }
    ...
    public interface OnGestureLockViewListener {
        void passwordList(List<Integer> passwordList);
    }

    public void setMyChoose(List<Integer> dataList){
        if(dataList!=null){
            this.mChoose = dataList;
        }
    }
}  